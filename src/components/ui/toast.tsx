...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast bg-popover text-popover-foreground focus-visible:border-ring focus-visible:ring-ring/50 pointer-events-auto absolute top-0 right-0 z-[calc(1000-var(--toast-index))] w-full origin-top rounded-2xl border shadow-lg will-change-transform outline-none select-none focus-visible:ring-[3px]",
...
const toastIconClass: Record<string, string> = {
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    icon = (
      <CircleCheckIcon className={toastIconClass.success} aria-hidden="true" />
    )
  }

  if (type === "info") {
    icon = <InfoIcon className={toastIconClass.info} aria-hidden="true" />
  }

  if (type === "warning") {
    icon = (
      <TriangleAlertIcon
        className={toastIconClass.warning}
        aria-hidden="true"
      />
    )
  }
  ...
  if (type === "error") {
    icon = <OctagonXIcon className={toastIconClass.error} aria-hidden="true" />
  }

  if (type === "loading") {
    icon = (
      <Loader2Icon
        className={cn("animate-spin", toastIconClass.loading)}
        aria-hidden="true"
      />
    )
  }
...
function ToastList() {
  ...
  return toasts.map((toastItem) => (
    <Toast
      key={toastItem.id}
      toast={toastItem}
      className={toastItem.type ? toastTypeClass[toastItem.type] : undefined}
    >
      <ToastContent>
